...
g <-  g +
  geom_fruit(data = filter(coex, sex=="female"), geom="geom_tile", aes(y=label,fill=coex.score), pwidth = 1) +
  scale_fill_gradient2(low="darkgray",high="blue",name="female coex. score") +
  geom_fruit(data = filter(coex, sex=="female" & significant_x), geom="geom_point", aes(y=label), color="blue", offset = 0.05)

  
g <- g + new_scale_fill() +
  geom_fruit(data = filter(coex, sex=="male"), geom="geom_tile", aes(y=label,fill=coex.score), pwidth = 1, offset=0.05) +
  scale_fill_gradient2(low="darkgray",high="red", name="male coex. score")  +
  geom_fruit(data = filter(coex, sex=="male" & significant_x), geom="geom_point", aes(y=label), color="red", offset = 0.05)
...
g <- g + fruitlist
  ...
  #geom_tiplab(aes(label=label),offset = 15.2)
