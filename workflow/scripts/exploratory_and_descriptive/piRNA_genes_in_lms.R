...
res <- mods %>%
  ...
  dplyr::select(model, gene_symbol, feature.y, class, estimate.qnorm, significant_model, significant_x)


#  ggplot(aes(str_wrap(class, width=10),estimate.qnorm)) +
#  geom_boxplot() +
#  #geom_violin(draw_quantiles = c(0.5), linewidth=1.5) +
#  facet_wrap(~model, scales="free_y") +
#  ggpubr::stat_compare_means(label.y.npc = 0.9) +
#  ylab("abs(TE coex. score)")
