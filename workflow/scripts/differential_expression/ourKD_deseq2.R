...
colData(se) <- colData(se) %>% 
  ...
  mutate(batch=case_when(str_detect(path_r1,"novo_")~"batch1",
                        ...
                        str_detect(path_r1,"novo4")~"batch4")) %>%
...
se <- se[,se$driver %in% c("aTub","tj","Mef2.R","nos")]
...
adjusted <- ComBat_seq(counts = mat, 
                       batch = se$batch,
                       group = se$knockdown,
                       covar_mod = model.matrix( ~ tissue , data=colData(se)),
                       shrink.disp = T,
...
contrasts <- list(CG16779.head = c("knockdown2","CG16779_female_head_Mef2.R","control_female_head_Mef2.R"),
                  ...
                  pan.tj.ovary = c("knockdown2","pan_female_gonad_nos","control_female_gonad_nos"),
...
dds.ovary_nos <- DESeqDataSet(se[,str_detect(se$tissue,"female_gonad") & se$driver == "nos"],~knockdown2)
dds.ovary_nos$knockdown2 <- relevel(dds.ovary_nos$knockdown2, ref = "control_female_gonad_nos")
...
dds_list_indiv <- list(split.testis.raw=dds.testis,
                 ...
                 split.ovary.nos.raw=dds.ovary_nos,
